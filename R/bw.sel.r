...
## Author: Binbin Lu
gwr.cv <- function(
    bw, X, Y, kernel = "bisquare", adaptive = FALSE,
    dp.locat, p = 2, theta = 0, longlat = F, dMat, verbose = T, parallel.method = F, parallel.arg = NULL)
{
  ...
  if (parallel.method == FALSE) {
    ...
    if (!all(is.infinite(gw.resi))) {
      CV.score <- sum(gw.resi)
    } else {
      CV.score <- Inf
    }
  } else {
...
gwr.cv.contrib <- function(
    bw, X, Y, kernel = "bisquare", adaptive = FALSE,
    dp.locat, p = 2, theta = 0, longlat = F, dMat, parallel.method = F, parallel.arg = NULL)
{
  dp.n <- length(dp.locat[, 1])
  ######### Distance matrix is given or not
...
gwr.aic <- function(bw, X, Y, kernel = "bisquare", adaptive = FALSE, dp.locat, p = 2, theta = 0, longlat = F, dMat, verbose = T, parallel.method = F, parallel.arg = NULL)
{
  ...
  if (verbose) {
    if (adaptive) {
      cat("Adaptive bandwidth (number of nearest neighbours):", bw, "AICc value:", AICc.value, "\n")
    } else {
      cat("Fixed bandwidth:", bw, "AICc value:", AICc.value, "\n")
    }
  }
  AICc.value
}
...
gwr.bic <- function(bw, X, Y, kernel = "bisquare", adaptive = FALSE, dp.locat, p = 2, theta = 0, longlat = F, dMat, verbose = T, parallel.method = F, parallel.arg = NULL)
{
  ...
  if (verbose) {
    if (adaptive) {
      cat("Adaptive bandwidth (number of nearest neighbours):", bw, "BIC value:", BIC.value, "\n")
    } else {
      cat("Fixed bandwidth:", bw, "BIC value:", BIC.value, "\n")
    }
  }
  BIC.value
}
