                      ...
                      parallel.arg = NULL)
{
  ...
  GW.arguments <- list(
    formula = formula, rp.given = rp.given, hatmatrix = hatmatrix, bw = bw,
  ...
  if (hatmatrix) {
    if (is.null(W.vect)) {
      ...
      colnames(gwres.df) <- c(c(
        c(colnames(betas), c("y", "yhat", "residual", "CV_Score", "Stud_residual")),
        paste(colnames(betas), "SE", sep = "_")
      ), paste(colnames(betas), "TV", sep = "_"), "Local_R2")
    } else {
      gwres.df <- data.frame(betas, y, yhat, residual, CV, Stud_residual, betas.SE, betas.TV, W.vect, local.R2)
      colnames(gwres.df) <- c(c(
        c(colnames(betas), c("y", "yhat", "residual", "CV_Score", "Stud_residual")),
        paste(colnames(betas), "SE", sep = "_")
      ), paste(colnames(betas), "TV", sep = "_"), "E_weigts", "Local_R2")
    }
  } else {
    if (is.null(W.vect)) {
  ...
  ##############
  res <- list(
    GW.arguments = GW.arguments, GW.diagnostic = GW.diagnostic, lm = lms, SDF = SDF,
    ...
    adaptive = FALSE, p = 2, theta = 0, longlat = F, dMat, W.vect = NULL, nperm = 999)
{
  ...
  lms$x <- x
  lms$y <- y
  gTSS <- c(cov.wt(matrix(y, ncol = 1), wt = rep(
    as.numeric(1),
    dp.n
  ), method = "ML")$cov * dp.n)
  if (missing(dMat)) {
