                      ...
                      kernel = "bisquare", adaptive = FALSE, p = 2, theta = 0, longlat = F, dMat, dMat.rp)
{
  ...
  p4s <- as.character(NA)
  hatmatrix = ifelse(diagnostic, TRUE, FALSE)
  ...
  model <- gwr.mixed.2.fast(x1, x2, y,
  ...
  if (hatmatrix) {
    ...
    edf <- gwr.mixed.trace.fast(x1, x2, y,
      adaptive = adaptive, bw = bw,
      kernel = kernel, dMat = dMat)
    
    model2 <- gwr.mixed.2.fast(x1, x2, y,
      adaptive = adaptive, bw = bw,
...
gwr.q.fast <- function(x, y, adaptive = F, bw, kernel, dMat)
{
  if (missing(dMat)) {
